...
const ProjectTables = () => {
  ...
  const delieveryManagement = (value, id) => {
    ...
    dispatch(OrderStatusUpdate(value, id)).then(() =>
  ...
  return (
    ...
    >
      <Card>
        <CardBody>
          <Heading sixe="2xl" textAlign="center" mb="20px" tag="h5">
            ORDER TRACKING
          </Heading>
                ...
                {OrderData.length == 0 ? (
                  ...
                  OrderData?.map((tdata, index) => (
                    <Tr key={index} mb="20px" border="2px solid lightgrey">
                      <Td>
                        <Flex flexDir="column" gap={4} fontFamily="fantasy">
                          <h1>
                      ...
                      <Td>
                        <Select
                          ...
                          bg={colorState(tdata.orderStatus)}
                          value={tdata.orderStatus}
                          onChange={({ target }) =>
                            delieveryManagement(target.value, tdata._id)
                          }
                        >
                      ...
                      <Td>
                        {tdata.productDetails?.reduce(
                          (acc, ele) =>
                            acc +
                            ele.productID?.approx_price_EUR * 70 * ele.count,
                          0
                        )}
                      </Td>
                    </Tr>
